...
def main():
    ...
    # ------------------------------------------------------------------
    # Additional statistics: bust probability and min non-zero multiplier
    # ------------------------------------------------------------------
    bust_weight = sum(weight for _, weight, m in csv_data if m == 0)
    bust_prob = bust_weight / stats['total_weight'] if stats['total_weight'] > 0 else 0.0
    print(f"  - Bust probability: {bust_prob:.6f} ({bust_prob*100:.4f}%)")

    # Check min/max multiplier requirements
    nonzero_multipliers = [m for _, _, m in csv_data if m > 0]
    min_nonzero = min(nonzero_multipliers) if nonzero_multipliers else None
    min_nonzero_x = (min_nonzero / stats['scale']) if min_nonzero is not None else 0.0
    max_x = stats['max_multiplier'] / stats['scale']

    print(f"  - Min non-zero multiplier (scaled): {min_nonzero_x:.2f}x")
    print(f"  - Max multiplier (scaled): {max_x:.2f}x")
    
    if min_nonzero is None or min_nonzero_x < 1.01:
        print(f\"[FAIL] Min non-zero multiplier is below 1.01x\")
        sys.exit(1)
